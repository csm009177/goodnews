...
4. 성경 기능 (bible, bible)
 ...
 [x] 한글 / 영문 / 같이보기 토글
 [x] 성서, 장, 구절 찾기 기능 (책/장 이동 + 구절 찾기 탭)
 [x] 장 단위 렌더링 (3개 장 슬라이딩 윈도우)
 [x] 좌→우 / 우→좌 길게 스크롤로 이전/다음 장 이동
 [x] 모바일 대응 : 터치 마킹, 길게 눌러 복사
 [x] 메모리 관리 : 범위 벗어난 장 DOM 제거
...
6. 일정 / 공지 기능 (schedule, schedule)
 ...
 [x] 기관장 권한 : 공지 등록/상단 고정(Pin)
 [x] 달력 모드 UI
 [x] 스트림 모드 UI
 [x] 달력/스트림 토글
 [x] 일정 생성/수정/삭제 기능 (폼 모달, 색상 선택, CRUD 연동)
...
8. 악보 기능 (sheet, sheet)
 ...
 [x] 악보 캐시 로더
9. 합창 기능 (choir, choir)
 [x] Pusher WebSocket 연결/해제 (합창모드 진입/이탈 시)
 [x] 지휘자 화면 미러링 (실시간 동기화)
 [x] 합창단 출석 로그/실시간 집계
 [x] Canvas 필기 : 지휘자/파트장 권한 악보 필기/그림
 [x] 녹음 기능 : 녹음 컨트롤러
 [x] 필기/녹음 데이터 R2 저장 (Presigned URL 기반 업로드, 진행률 표시)
10. 권한 체계
 [x] 인증 시스템 (AuthProvider, localStorage 세션 지속)
 [x] 로그인/로그아웃 모달 (데모 계정 지원)
 [x] 사용자 메뉴 (역할 기반 메뉴 항목)
